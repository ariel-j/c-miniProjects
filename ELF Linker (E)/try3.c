...
void update_undefined_symbols(ElfFile* file1, ElfFile* file2, Elf32_Shdr* new_sections);
...
void merge_elf_files() {
    ...
    for (int i = 0; i < header1->e_shnum; i++) {
        if (new_sections[i].sh_addralign > 1) {
        ...
        if (sections1[i].sh_type == SHT_PROGBITS && (
            ...
            write(out_fd, new_symtab, sections1[i].sh_size);
            free(new_symtab);
