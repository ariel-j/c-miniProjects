...
_start:
    ...
    pop dword ecx                           ; Get argc
    mov esi, esp                            ; Get argv
...
parse_args:
    ...
    mov edx, 1                              ; length = 1
    int 0x80                                ; write syscall (is it in other words = EXECUTE?)
...
not_input:
    ; Check for -o flag
    cmp byte [edx], '-'                     ; (arg[i][0] == '-')
    jne next_arg                            ; if not -> next arg
    cmp byte [edx + 1], 'o'                 ; (arg[i][1] == 'o')
    jne next_arg                            ; if not -> next arg
    
    ; Open output file
    mov eax, 5                              ; sys_open
    lea ebx, [edx + 2]                      ; filename (skip -o)
    mov ecx, 0x241                          ; O_WRONLY | O_CREAT | O_TRUNC -> read write only, if not existed create it, and reset it each time we call it
    mov edx, 0644o                          ; File permissions --- ????
    int 0x80
    
    cmp eax, 0                              ; Check if open failed ->> why does that check it?
    jl open_error                           ;
